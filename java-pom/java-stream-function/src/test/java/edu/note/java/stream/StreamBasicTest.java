...
public class StreamBasicTest {
    ...
    void testStreamToList() {
        ...
        assertArrayEquals(new Integer[]{1, 2, 3, 4, 5}, list.stream().map(Integer::parseInt).toArray());
    }
    ...
    void test03() {
        ...
        assertEquals(3, list2.stream().distinct().count());

        // concat: 合并a和b两个流为一个流
        assertEquals(7, Stream.concat(list.stream(), list2.stream()).count());
    }
