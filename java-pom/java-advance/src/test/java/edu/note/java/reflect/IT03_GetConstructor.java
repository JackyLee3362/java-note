...
public class IT03_GetConstructor {
    ...
    void test1() {
        // public
        Assertions.assertEquals(3, clazz.getConstructors().length);
    ...
    void test2() throws NoSuchMethodException {
        ...
        Assertions.assertEquals(Modifier.PUBLIC, clazz.getConstructor(String.class).getModifiers());
        Assertions.assertThrows(NoSuchMethodException.class, () -> clazz.getConstructor(int.class, int.class));
        // 获取 private 构造方法
        Assertions.assertEquals(Modifier.PRIVATE, clazz.getDeclaredConstructor(int.class, double.class).getModifiers());
    }
    ...
    void test3() throws Exception {
        ...
        Assertions.assertEquals(con4.getModifiers(), Modifier.PRIVATE);

        Parameter[] params = con4.getParameters();
        Assertions.assertEquals(params.length, 2);
        Assertions.assertEquals(params[0].getName(), "arg0");
        Assertions.assertEquals(params[1].getName(), "arg1");

        // ! 暴力反射：表示临时取消权限校验
        // private 无法直接执行，会抛出异常
        Assertions.assertThrows(IllegalAccessException.class, () -> con4.newInstance(18, 1.88));
        con4.setAccessible(true);
        Assertions.assertNotNull((Student) con4.newInstance(18, 1.88));
    }
}